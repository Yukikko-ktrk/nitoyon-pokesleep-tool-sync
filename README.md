...
読取に使うポケモン・食材の索引データは、起動時 (1日1回) に nitoyon 氏のリポジトリから
最新を取得して自動更新される (v3.1.0〜)。ツールに新ポケモンや新食材が追加されても、
スクリプトの更新なしで取込対象になる。取得に失敗した場合はスクリプト埋め込みのデータで動く。
例外として、新しい「フォーム」(キャプテンのような姿違い) や新サブスキルの登場時だけは
シリアル仕様側の対応が必要なためスクリプト更新で追従する。

### 読取が混雑エラーになったとき (v3.1.1〜)

Gemini / Claude 側が混雑していて 503 等の一時エラーになった場合は、自動で最大4回まで
再試行する。それでも回復しないときは、設定を変えずに別の Gemini モデルへ一時的に
切り替えて1回だけ読取を試みる。結果欄に出る「失敗した N 枚を再試行」ボタンを押せば、
成功した分はそのままに、失敗した画像だけ選び直さずにやり直せる。
既定モデルは Gemini 3.8 Flash になった。以前から Gemini 3.5 Flash を保存していた端末は
そのまま使えるが、API設定タブの案内から切り替えられる。
