...
### 使い方の注意: 個体を開いたまま取り込むと (v3.0.2 以前)

v3.0.2 以前は、ボックスで個別のポケモンを選んでステータス (編集画面) を開いた状態の
ままスクショを読み込んで登録すると、新規追加ではなく**開いていた個体を読み取り結果で
上書きしてしまう**問題があった。v3.0.3 からは取込時にボックスの選択を自動で解除する
ため、常に新規追加になる。
