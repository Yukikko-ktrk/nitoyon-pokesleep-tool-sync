...
このツールは PWA (`display: standalone`) のため、**iPhone の「ホーム画面に追加」から開くと Safari 機能拡張が動かず、localStorage も Safari と別になる**ので同期できない。
iPhone で1タップ起動したい場合は、ショートカットアプリの「URL を開く」で Safari から開くショートカットを作ってホーム画面に置くか、Safari のブックマークを使う。
PC (Edge / Chrome) の「アプリとしてインストール」はブラウザとプロファイルを共有するため問題なく動く。
