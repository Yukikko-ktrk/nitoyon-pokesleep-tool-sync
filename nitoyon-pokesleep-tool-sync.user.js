...
(function () {
	...
	function buildData(pokemonJson, jaPokemonsJson, jaDataJson) {
		...
		for (const p of pokemonJson) {
			try {
				const ja = jaNames[p.name];
				if (!ja) throw new Error('日本語名がありません');
				const form = p.form === undefined ? 0 : FORM_MAP[p.form];
				if (form === undefined) throw new Error(`未知のフォーム ${p.form}`);
				const ing1 = ingName(p.ing1);
				if (!ing1) throw new Error('第1食材がありません');
				const row = [p.id, ja, form, ing1, ingName(p.ing2), ingName(p.ing3),
					p.mythIng ? 'M' : '', p.evolutionCount];
				if (p.mythIng) row.push(p.mythIng.map((m) => ingJa[m.name]));
				pokemons.push(row);
			} catch (e) {
				skipped.push(`${p.name}: ${e.message}`);
			}
		}
		if (skipped.length > 0) console.warn('[pst] 索引データ: 取込対象外', skipped);
		if (pokemons.length < DATA.pokemons.length) {
			throw new Error(`取得データが埋め込み版より少ない (${pokemons.length} < ${DATA.pokemons.length})`);
		}
		return { pokemons, ingredients: Object.values(ingJa).sort() };
	}

	function adoptData(built, label) {
		DATA = { ...DATA, pokemons: built.pokemons, ingredients: built.ingredients };
		console.log(`[pst] 索引データ: ${label} (${built.pokemons.length}種)`);
	}

	function initDataAutoUpdate() {
		const cache = loadJson(DATA_CACHE_KEY);
		const cacheValid = cache && Array.isArray(cache.pokemons) &&
			cache.pokemons.length >= DATA.pokemons.length &&
			cache.pokemons.every((r) => Array.isArray(r) && r.length >= 8) &&
			Array.isArray(cache.ingredients);
		if (cacheValid) adoptData(cache, 'キャッシュを採用');
		if (cacheValid && Date.now() - (cache.fetchedAt || 0) < DATA_TTL_MS) return;
		(async () => {
			try {
				const [pk, jp, jd] = await Promise.all(DATA_FILES.map(async (path) => {
					const res = await fetch(DATA_RAW_BASE + path);
					if (!res.ok) throw new Error(`${path} ${res.status}`);
					return res.json();
				}));
				const built = buildData(pk, jp, jd);
				adoptData(built, '最新版を取得');
				saveJson(DATA_CACHE_KEY, { fetchedAt: Date.now(), ...built });
			} catch (e) {
				console.warn('[pst] 索引データの自動更新に失敗 (現行データで継続)', e);
			}
		})();
	}
	...
	const Serial = { DATA, buildData, findPokemon, ingredientType, encode, decode,
		adoptData, get currentData() { return DATA; } };
